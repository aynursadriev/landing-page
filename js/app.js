    ...
    (() => {
        ...
        function formSubmit(options = {
            ...
            async function formSubmitAction(form, e) {
                ...
                if (error === 0) {
                    ...
                    if (ajax) {
                        e.preventDefault();
                        grecaptcha.execute("6LcUuPEpAAAAAKJWyLec29EaPW0U94MffyKRl67h", {
                            action: "submit"
                        }).then((async function(token) {
                            const recaptchaResponse = form.querySelector("#recaptchaResponse");
                            ...
                            try {
                                ...
                                if (response.ok) {
                                    ...
                                    if (responseResult.success) formSent(form); else formError(responseResult.message);
                                } else formError("Ошибка при отправке формы");
                            } catch (error) {
                                formError("Ошибка сети");
                            } finally {
            ...
            function formError(message) {
                ...
                setTimeout((() => {
                    if (flsModules.popup) flsModules.popup.open("#formErrorMessage");
                }), 0);
                formLogging(`Ошибка: ${message}`);
            }
        ...
        const observer = new MutationObserver(((mutationsList, observer) => {
            for (let mutation of mutationsList) if (mutation.type === "attributes" && mutation.attributeName === "class") {
                ...
                if (document.documentElement.classList.contains("lock")) {
                    const bodyPaddingRight = parseInt(getComputedStyle(document.body).paddingRight, 10);
                    header.style.width = `calc(100% - ${bodyPaddingRight}px)`;
        ...
        document.addEventListener("DOMContentLoaded", (function() {
            ...
            window.addEventListener("scroll", (function() {
                ...
                const scrollPercentage = scrollDistance / (documentHeight - windowHeight) * 100;
                if (scrollPercentage >= 90) scrollBtn.classList.add("btn-up"); else scrollBtn.classList.remove("btn-up");
            }));
        }));
        window["FLS"] = false;
        isWebp();
